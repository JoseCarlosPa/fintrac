      ...
      >
        ...
        <div className="flex flex-col space-y-3">
          ...
          <span
            ...
            <span
              className="ml-2">({(parseFloat(card?.maxAmount) - parseFloat(card?.usedAmount)).toLocaleString('es-MX', {
              style: 'currency',
              currency: 'MXN'
            })})
            </span>
                <div className="mt-1 h-2 rounded-r w-full bg-neutral-200 dark:bg-neutral-600">
                      <div className="h-2 rounded-r bg-blue-800"
                           style={{'width': `${((parseFloat(card.usedAmount) * 100) / parseFloat(card.maxAmount)).toFixed(2)}%`}}></div>
                </div>
            <span
              className="flex flex-row justify-center mt-2">{((parseFloat(card.usedAmount) * 100) / parseFloat(card.maxAmount)).toFixed(2)}%</span>
            </span>
